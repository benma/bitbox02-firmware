 ...
 * Calls keystore_secp256k1_antiklepto_commit, but it caches the keypath and msg32 to be used when
 * signing.
 * Must use `keystore_antiklepto_clear` after signing or aborting the signing process.
...
/**
 * Clears the signing data. Must be called at some point after
 * `keystore_antiklepto_secp256k1_commit`, either after `keystore_antiklepto_secp256k1_sign` or
 * instead of it if it will never be called (signing process aborted).
 */
