...
use api::error::{Context, Error, ErrorKind};
...
pub async fn next_request(
    ...
    noise::encrypt(&api::encode(response), &mut out).error_kind(ErrorKind::NoiseEncrypt)?;
    ...
    match request.split_first() {
        Some((&noise::OP_NOISE_MSG, encrypted_request)) => {
            let decrypted_request =
                noise::decrypt(&encrypted_request).error_kind(ErrorKind::NoiseDecrypt)?;
