...
pub trait Xpub: Sized {
    ...
    fn from_keypath<H: crate::hal::Hal>(
        hal: &mut H,
        keypath: &[u32],
        compute: Compute,
    ) -> Result<Self, ()>;
...
impl<X: Xpub + Clone> XpubCache<X> {
    ...
    fn cache_get_set<H: crate::hal::Hal>(
        &mut self,
        hal: &mut H,
        keypath: &[u32],
    ) -> Result<X, ()> {
    ...
    pub fn get_xpub<H: crate::hal::Hal>(
        &mut self,
        hal: &mut H,
        keypath: &[u32],
    ) -> Result<X, ()> {
...
mod tests {
    ...
    fn test_xpub_cache() {
        ...
        assert_eq!(cache.get_xpub(&mut hal, &[]).unwrap().0.as_slice(), &[]);
        ...
        *ROOT_DERIVATIONS.borrow_mut() = 0;
        ...
        assert_eq!(
            cache
                .get_xpub(&mut hal, &[84 + HARDENED, 0 + HARDENED, 0 + HARDENED, 1, 2])
                .unwrap()
                .0
                .as_slice(),
        ...
        *CHILD_DERIVATIONS.borrow_mut() = 0;
        ...
        assert_eq!(
            cache
                .get_xpub(&mut hal, &[84 + HARDENED, 0 + HARDENED, 0 + HARDENED, 0, 0])
                .unwrap()
                .0
                .as_slice(),
    ...
    fn test_bip32_xpub_cache() {
        ...
        assert_eq!(
            &cache
                .get_xpub(&mut hal, &[84 + HARDENED, 0 + HARDENED, 0 + HARDENED, 1, 3])
                .unwrap()
                .serialize_str(bip32::XPubType::Xpub)
                .unwrap(),
