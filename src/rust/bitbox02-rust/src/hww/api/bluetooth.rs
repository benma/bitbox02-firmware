...
async fn _process_upgrade<F: Funcs>(
...
async fn process_upgrade(request: &pb::BluetoothUpgradeInitRequest) -> Result<Response, Error> {
    struct RealFuncs;
    ...
    let response = _process_upgrade(request, RealFuncs, ALLOWED_HASH).await;

    if response.is_ok() {
        status::status("Upgrade\nsuccessful", true).await;
    } else {
        status::status("Upgrade failed", false).await;
    }
...
pub async fn process_api(request: &Request) -> Result<Response, Error> {
    ...
    match request {
        Request::UpgradeInit(ref request) => process_upgrade(request).await,
