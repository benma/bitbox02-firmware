...
pub fn process_is_script_config_registered(
    ...
    match request.registration.as_ref() {
        ...
        Some(pb::BtcScriptConfigRegistration {
            ...
            script_config:
                Some(pb::BtcScriptConfig {
                    config: Some(Config::Descriptor(descriptor)),
                }),
            ..
        }) => {
            let coin = BtcCoin::from_i32(*coin).ok_or(Error::InvalidInput)?;
            Ok(Response::IsScriptConfigRegistered(
                pb::BtcIsScriptConfigRegisteredResponse {
                    is_registered: super::descriptors::get_name(coin, descriptor)?.is_some(),
                },
            ))
        }
    ...
    match request.registration.as_ref() {
        ...
        Some(pb::BtcScriptConfigRegistration {
            ...
            super::descriptors::confirm(
                ...
                &name,
                descriptor,
                super::descriptors::Mode::Advanced,
            )
            .await?;
        ...
        // Only multisig and descriptor registration supported for now.
        _ => Err(Error::InvalidInput),
    }
}
