...
fn parse_recipient(recipient: &[u8]) -> Result<[u8; 20], Error> {
    recipient.try_into().or(Err(Error {
        msg: Some(format!("failed parsing recipient: {:?}", recipient)),
...
pub async fn process(request: &pb::EthSignRequest) -> Result<Response, Error> {
    let params = params_get(request.coin as _)
        ...
        || request.data.len() > 1024
    {
        return Err(Error {
            msg: Some(format!(
                "sizes too big: nonce({}), gas_price({}), gas_limit({}), value({}), data({})",
                request.nonce.len(),
                request.gas_price.len(),
                request.gas_limit.len(),
                request.value.len(),
                request.data.len(),
            )),
            kind: ErrorKind::InvalidInput,
