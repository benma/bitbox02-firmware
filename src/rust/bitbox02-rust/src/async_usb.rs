...
//! USB response. Terminology: host = computer, client = BitBox02.
    ...
    F: core::future::Future<Output = UsbOut> + 'static,
{
    ...
    match *state {
        UsbTaskState::Nothing => {
            ...
            *state = UsbTaskState::Running(Some(task), WaitingForNextRequestState::NotWaiting);
        }
...
pub fn on_next_request(usb_in: &[u8]) {
    ...
    match *state {
        UsbTaskState::Running(
            ...
            *next_request_state = WaitingForNextRequestState::NotWaiting;
        }
