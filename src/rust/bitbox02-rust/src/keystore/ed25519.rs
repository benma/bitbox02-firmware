...
mod tests {
    ...
    fn test_get_seed() {
        ...
        assert_eq!(
            get_seed(&mut TestingHal::new()).unwrap().as_slice(),
    ...
    fn test_get_xpub() {
        crate::keystore::lock();
        {
            ...
            assert!(get_xpub(&mut hal, &[]).is_err());
        }

        mock_unlocked();

        let xpub = get_xpub(&mut TestingHal::new(), &[]).unwrap();
        ...
        let xpub = get_xpub(&mut TestingHal::new(), &[10 + HARDENED_OFFSET, 10]).unwrap();
    ...
    fn test_get_xprv() {
        ...
        {
            let mut hal = TestingHal::new();
            assert!(get_xprv(&mut hal, &[]).is_err());
        }

        mock_unlocked();
        let xprv = get_xprv(&mut TestingHal::new(), &[]).unwrap();
        ...
        let xprv = get_xprv(&mut TestingHal::new(), &[10 + HARDENED_OFFSET, 10]).unwrap();
    ...
    fn test_sign() {
        ...
        {
            let mut hal = TestingHal::new();
            assert!(sign(&mut hal, &[10 + HARDENED_OFFSET, 10], msg).is_err());
        }

        mock_unlocked();
        let sig = sign(&mut TestingHal::new(), &[10 + HARDENED_OFFSET, 10], msg).unwrap();
